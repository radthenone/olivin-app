...
- `backend/src/apps/orders` — `Cart` (użytkownik **albo** `session_key` gościa, ograniczenie w bazie; token wydawany przy pierwszym dodaniu pozycji, odsyłany nagłówkiem `X-Cart-Token`) i `CartItem` (wariant, ilość ≤ 5, grawerunek, `second_size`/`second_engraving_text` dla pary). Koszyk **nie** zamraża ceny — wycena idzie z wariantu przy odczycie. Para tylko z wyrobu na zamówienie (ADR 0024). API `AllowAny`, bez stronicowania: `GET /cart/`, `POST /cart/items/`, `PATCH`/`DELETE /cart/items/{id}/`, `POST /cart/merge/` (tylko zalogowany); każda odpowiedź to cały koszyk z `discountAmount`/`couponAmount` (na razie zerowe). Zadanie beat `purge_stale_guest_carts` co dobę kasuje koszyki gości bez aktywności 30 dni. Testy w `backend/src/tests/orders/`.
